...
def index():
    response = "Hello, use 'http://currentIP/rest/api/ingredient/input_id' to get the ingredient info \nuse 'http://currentIP/rest/api/search/input_string' to get search results"
    return response
...
def search():
   ingredient_name = request.headers.get('name')
   response = getNameResults(ingredient_name)
   return jsonify(response)
if __name__ == '__main__':
    #app.run(host='0.0.0.0')
    print getIdResults(1001)
